...
export function ExpensesSection({ expenses, onUpdate }: ExpensesSectionProps) {
  ...
  const addItem = () => {
    ...
    onUpdate([...expenses, newItem]);
    setNewItemName('');
  };

  const updateItem = (id: string, field: keyof ExpenseItem, value: string | number | boolean) => {
    onUpdate(
      expenses.map((item) => {
        if (item.id === id) {
  ...
  return (
    <Card className="bg-pink-50/50">
      <CardHeader>
        <CardTitle className="text-center uppercase tracking-wide text-pink-900">Expenses</CardTitle>
      </CardHeader>
      <CardContent>
